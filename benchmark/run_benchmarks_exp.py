...
VMMALLOC_PATH = "/usr/local/lib/libvmmalloc.so"
...
def getCacheFileName(binary, args):
    if "with_jemalloc" in args: with_jemalloc = True
    if "with_vmmalloc" in args: with_vmmalloc = True
    return os.path.join(os.getcwd(), "cached", "%s_%d_%d_%d_%d%s%s" % (binary,
                                                                      args["threads_min"],
                                                                      args["threads_max"],
                                                                      args["payload_min"],
                                                                      args["payload_max"],
                                                                      "_jemalloc" if with_jemalloc else "",
								      "_vmmalloc" if with_vmmalloc else ""))

def runBenchmarkBinary(binary, parameters, with_jemalloc, with_vmmalloc, node=0, runs=5, miliseconds=False):
    ...
    if with_jemalloc:
        procString = "LD_PRELOAD=%s %s" % (VMMALLOC_PATH, procString)
    env = {"LD_LIBRARY_PATH": os.path.join(os.getcwd(), "..")}
    elapsed = 0.0
    global verbose
...
def runBenchmark(binary, args):
    cachefile = getCacheFileName(binary, args)
...
def plotBenchmark(benchname, args):
    ...
    if args["with_jemalloc"]:
        ...
        plt.plot(plotX, runBenchmark("bench_%s" % benchname, args), label="jemalloc", ls=":", color="black")

    # if selected, run with vmmalloc
    if args["with_vmmalloc"]:
        print "Running '%s' for vmmalloc" % benchname
        plt.plot(plotX, runBenchmark("bench_%s" % benchname, args, False, True), label="vmmalloc", ls=":", color="black")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--with-vmmalloc", action="store_true", help="include a run with vmmalloc in the benchmark", default=True)
    ...
    parser.add_argument("--with-makalu", action="store_true", default=True)
