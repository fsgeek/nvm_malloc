...
inline void* initialize(const std::string workspace_path, int recover_if_possible) {
...
#elif USE_PMDK
    std::string workspace_file = "pmcto-test";
    std::string test_path = workspace_path + workspace_file;
	...
	assert(retry++ < 10);
        ...
        pcp = pmemcto_create(path.c_str(), layout_name, pool_size, 0600);
    }
    return (PMEMctopool *)0;
#elif USE_PMOBJ
    std::string workspace_file = "pmobj-test";
    std::string path = workspace_path + workspace_file;
    static const char *layout_name = "bench";
    static const size_t pool_size = (size_t)10 * 1024 * 1024 * 1024; 
    int retry = 0;
    ...
    while (NULL == pop) {
        ...
        pop = pmemobj_create(path.c_str(), layout_name, pool_size, 0600);
    }
    assert(NULL != pop);
    ...
    pmem_baseaddr = pmem_map_file(path.c_str(), pool_size, PMEM_FILE_CREATE,
